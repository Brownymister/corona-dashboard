...
class TestScrape(unittest.TestCase):
    ...
    def test_render_chart(self):
        ...
        Scrape_setup().setup_render_chart(path)
        scrape = Scrape()
        ...
        scrape.render_chart(all_data_in_db, 1, path + 'InfectionPerDayChart', 'Infektionen Pro Tag')
        scrape.render_chart(all_data_in_db, 2, path + 'TotalInfectionChart', 'Corona Infektionen in Millionen')
        scrape.render_chart(all_data_in_db, 3, path + 'procent', 'Corona Infektionen in Millionen')
        scrape.render_chart(all_data_in_db, 4, path + 'incidence', 'incidence')
        scrape.render_chart(all_data_in_db, 5, path + 'TotalDeathChart', 'Tode durch das Corona Virus')
        scrape.render_chart(all_data_in_db, 6, path + 'DeathPerDayChart', 'Tode durch das Corona Virus Pro Tag')
        scrape.render_chart(all_data_in_db, 8, path + 'recoveries', 'recoveries')
